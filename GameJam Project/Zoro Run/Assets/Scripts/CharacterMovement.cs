...
[System.Serializable]
public enum SIDE {Left = -14, Mid = 0, Right=14}
...
public class CharacterMovement : MonoBehaviour
{
    ...
    [HideInInspector]public float StumbleTolerance = 10f, stumbleTime;
    ...
    float x,y=0.0f;
    private float jogtt = 1.2f;
    private float landSpeed=2;
    private bool inJump, inRoll;
    private float ColHeight, ColCenterY;
    [HideInInspector] public bool isDeath = false;
    [HideInInspector] public GameManager scrpt3;
    [HideInInspector] public ScoreUI ScoreScript;
    private int Retry;
    private int score;
    ...
    {   
        ...
        ScoreScript = FindObjectOfType<ScoreUI>();
    ...
    void Update()
    {
        ...
        if(up){
            LockJump = true;
            StartCoroutine(WaitBeforeLocking());
        }
    ...
    public IEnumerator WaitBeforeLocking(){
    ...
    public IEnumerator Killed(){
        ...
        int x = ScoreScript.numOfCoins;
        PlayerPrefs.SetFloat("LastScore", transform.position.z);
        PlayerPrefs.SetFloat("LastCoinCount", x);
        if(!PlayerPrefs.HasKey("HighScore")){
            PlayerPrefs.SetFloat("HighScore", transform.position.z);
        }
        else{
            if(PlayerPrefs.GetFloat("HighScore") < transform.position.z){
                PlayerPrefs.SetFloat("HighScore", transform.position.z);
            }
        }
        if(!PlayerPrefs.HasKey("MaxCoins")){
            PlayerPrefs.SetFloat("MaxCoins", x);
        }
        else{
            if(PlayerPrefs.GetFloat("MaxCoins") < x){
                PlayerPrefs.SetFloat("MaxCoins", x);
            }
        }
        if(Retry == 1){
            FindObjectOfType<RetryUI>().PlayRetry();    
            Retry = 0;
        }
    }
    ...
    public void Jump(){
        if(controller.isGrounded){
            ...
            y -=JumpPower*landSpeed*Time.deltaTime;
            if(LockJump)
